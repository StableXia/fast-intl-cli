...
exports.exportUntranslatedMessages = exports.logUntranslatedMessages = exports.checkUntranslatedMessages = void 0;
...
function checkUntranslatedMessages(mode, options) {
...
function logUntranslatedMessages(messages, lang) {
    if (messages.length === 0) {
        view_1.log.primary(view_1.log.chalk.green(`在[${lang}]中未找到未翻译的文案`));
    }
    else {
        view_1.log.primary(view_1.log.chalk.bgRed.white(`在【${lang}】中找到【${messages.length}】处未翻译的文案：`));
    }
}
exports.logUntranslatedMessages = logUntranslatedMessages;
function exportUntranslatedMessages(messages, lang, outputPath) {
    const content = d3_dsv_1.tsvFormatRows(messages);
    const dir = path_1.default.resolve(outputPath);
